    ...
    if ($ObjBBDD->comprobarConexion()) {
        ...
        if (isset($_SESSION["idusuario"])) {
            if ($_SESSION["tipo"] == "b" || $_SESSION["tipo"] == "p") {
                $sql = 'SELECT * FROM temario WHERE idtemario='.$_POST["materia"];//consulta datos temario
                $resultado=$ObjBBDD->ejecutarConsulta($sql);
                $fila = $ObjBBDD->extraerFila($resultado);
                $sql2 = 'SELECT * FROM reunion WHERE activa=1 AND participante IS NULL AND temario='.$_POST["materia"];//consulta si hay reunion disponible
                ...
                if($ObjBBDD->filasObtenidas($resultado2) > 0){
                    ...
                    $sql = 'UPDATE reunion SET participante="' . $_SESSION["idusuario"] . '"WHERE idreunion="'.$fila2["idreunion"].'";';//consulta unir reunion
                    $ObjBBDD->ejecutarConsulta($sql);
                    header("Location:instart.php?r=".$room."#".$fila2["seed"]);
                }else{
                    $sql = 'INSERT INTO reunion (inicio,anfitrion,temario,seed) VALUES (NOW(), "' . $_SESSION["idusuario"] . '", "' . $_POST["materia"] . '", "' . $_COOKIE["hash"] . '");';//consulta crear reunion
