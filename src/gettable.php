<?php
    ...
    if($_SESSION["tipo"]=="b" || $_SESSION["tipo"]=="p"){
        $sql = "select * from reunion WHERE activa=0 AND (anfitrion=".$_SESSION["idusuario"]." OR participante=".$_SESSION["idusuario"].") ORDER BY activa DESC;";//obtener reuniones del usuario sesion
    }else{
        $sql = "SELECT * FROM reunion ORDER BY activa DESC;";//obtener todas las reuniones
    }
    ...
    if ($ObjBBDD->filasObtenidas($resultado) > 0) {
        ...
        while ($fila = $ObjBBDD->extraerFila($resultado)) {
            $sql2 = "select usuario from usuario WHERE idusuario=" . $fila['anfitrion'];//obtener nombre de usuario anfitrion
            ...
            if(is_null($fila["participante"])){
                ...
                $sql3 = "select usuario from usuario WHERE idusuario=" . $fila['participante'];//obtener nombre de usuario participante
                $resultado3=$ObjBBDD->ejecutarConsulta($sql3);
                $fila3 = $ObjBBDD->extraerFila($resultado3);
            }
            $sql5 = "select * from temario WHERE idtemario=" . $fila['temario'];//obtener nombre de temario
            $resultado5=$ObjBBDD->ejecutarConsulta($sql5);
            $fila5 = $ObjBBDD->extraerFila($resultado5);
            $sql4 = "select nombre from asignatura WHERE idasignatura=" . $fila5['asignatura'];//obtener nombre de asignatura
