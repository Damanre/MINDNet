<?php
    ...
    $ObjBBDD->conectar();
    $sql = "select * from mensaje WHERE reunion=".$_COOKIE['room']." ORDER BY fecha asc";//consulta mensajes
    $resultado=$ObjBBDD->ejecutarConsulta($sql);
    if ($ObjBBDD->filasObtenidas($resultado) > 0) {
        echo '<table>';
        while ($fila = $ObjBBDD->extraerFila($resultado)) {
            $sql2 = "select usuario from usuario WHERE idusuario=" . $fila['usuario'];//obtener nombre usuario del mensaje
    ...
    $sqlh = 'UPDATE reunion SET seed="' . $_COOKIE["hash"] . '"WHERE idreunion="'.$_COOKIE["room"].'";';//incluir hash a reunion
